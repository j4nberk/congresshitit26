...
# Instagram mobile API login + cookie persistence
...
# Cookie file stored next to this script
COOKIE_JSON_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "cookie.json")
...
def login_instagram(username: str, password: str, timeout_prelogin: int = 10, timeout_login: int = 20) -> Tuple[str, str, str, str]:
    ...
    if j.get("two_factor_required"):
        raise LoginError("Two-factor authentication required on this account; use a dummy account without 2FA.")
